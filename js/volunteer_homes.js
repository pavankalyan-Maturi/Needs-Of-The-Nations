...
firebase.database().ref('DonarsFoodInfo').on('value',function(snapshot){
    let users=snapshot.val();
  document.getElementById('Donarfoodtable').innerHTML='';
  let count=1;
  ...
  function func(){
    ...
    if (id != null) {
      ...
      firebase.database().ref('DonarsFoodInfo/'+id).remove()
      .then((info)=>{
  ...
  function download_Donarfood_data() {
    ...
    if(dd<10) dd='0'+dd;
    if(m<10) m='0'+m;
    const filename1="DonarFood_data"+"_"+dd+"/"+m+"/"+yy;
 kendo.drawing.drawDOM($("#donardata"))
     .then(function(group){
