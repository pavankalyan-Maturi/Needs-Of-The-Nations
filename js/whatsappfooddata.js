...
firebase.database().ref('whatsappFoodDonarInfo').on('value',function(snapshot){
    let users=snapshot.val();
    let count=1;
  document.getElementById('whatsappdata').innerHTML='';
  for(const user in users)
  {
    document.getElementById('whatsappdata').innerHTML+=`
    <tr>
    <td>${count}</td>
    <td>${users[user].Mobile_Number}</td>
    <td>${users[user].mailid}</td>
    <td>${users[user].UserName}</td>
    <td>${users[user].DonarType}</td>
...
count++;
  ...
  function func(){
    ...
    var id = prompt("Enter Mobile Number", "MobileNumber");
    if (id != null) {
      // var id=document.getElementById('oid').value;
      firebase.database().ref('whatsappFoodDonarInfo/'+id).remove()
      .then((info)=>{
...
// ----------------whatsapp foodinfo data download------------
function download_whatsapp_data(filename) {
  ...
  if(dd<10) dd='0'+dd;
  if(m<10) m='0'+m;
  const filename1="Whatsapp_food_data"+"_"+dd+"/"+m+"/"+yy;
kendo.drawing.drawDOM($("#table_download"))
   .then(function(group){
...
function sortTable(n) {
                var table;
                table = document.getElementById("table_download");
